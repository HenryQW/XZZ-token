...
describe("Test token contract", function () {
  ...
  describe("Transactions", function () {
    ...
    it("Should fail if sender doesn’t have enough tokens", async function () {
      ...
      ).to.be.revertedWith("InsufficientBalance(1, 0)");
  ...
  describe("Minting", function () {
    it("Should not be allowed for minting", async function () {

      const initialAddr1Balance = await hardhatToken.balanceOf(addr1.address);

      // Mint 100 tokens for addr1.
      await expect(hardhatToken.connect(addr1).mint(addr1.address, 100)).to.be.revertedWith("Minter not authorized");
